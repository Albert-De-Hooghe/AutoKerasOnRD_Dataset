...
def RD_Dataset_train_binary_classif(taille):
    ...
    for i in range(var1):    # len(train_csv.image)
        if path.exists(path_of_image_folder+'/'+train_csv.image[i]+'.png'):
            list.append([getImagefromImagePath(path_of_image_folder + '/' + train_csv.image[i]+'.png'), zeroOrOneForTheLabel(train_csv.level[i])]) #])#, train_csv.image[i]])
    ...
    #     list.append(getImagefromImagePath(path_of_image_folder+'/'+single_image_name))
...
#         if path.exists(path_of_image_folder+'/'+train_csv.image[i]+'.png'):
#             np.append(list,[getImagefromImagePath(path_of_image_folder + '/' + train_csv.image[i]+'.png'), zeroOrOneForTheLabel(train_csv.level[i])])
#
#
#     # function that return the list of (image, labels) from the path of the folder (image in the tensor format)
#
...
def RD_Dataset_test_binary_classif(taille):
    ...
    for i in range(var1,var2):    # len(train_csv.image)
        if path.exists(path_of_image_folder+'/'+train_csv.image[i]+'.png'):
            list.append([getImagefromImagePath(path_of_image_folder + '/' + train_csv.image[i]+'.png'), zeroOrOneForTheLabel(train_csv.level[i])])
...
def RD_Dataset_test_5_classes(taille):
    ...
    for i in range(var1, var2):  # len(train_csv.image)
        if path.exists(path_of_image_folder + '/' + str(train_csv.image[i]) + '.png'):
            list.append([getImagefromImagePath(path_of_image_folder + '/' + str(train_csv.image[i]) + '.png'),
                         train_csv.level[i]])
...
if __name__ == "__main__":
    ...
    # getImagefromImagePath("./testPreprocessed/11_left.png").shape# .shape nous donne torch.Size([3, 448, 448])
